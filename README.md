...
На самом деле выбор стоял между QLoRA для Qwen3-30-A3B и LoRA Qwen3-14B. Возиться с квантированием на обучении и МОЕ архитектурой, вообще не хотелось. А Qwen3-14B свободно помещался в 48Gi VRAM + оставалось много места для батча и всего остального.
...
- cosine - 0.63
- rouge1f - 0.3
- llm as jude - 3.815 (0 - 10)

## Хакаем награду
RLHF - оказалась очень интересная штука, все крутые чат модели, ризонинг это результат RLHF. При выборе метода я был ограничен видеокартой, временем и знаниями. В RLHF куча методов и каждый с какими-то наворотами, например, `PPO` или `RS`, требовали: `reward` модель, `critic` модель, `reference` модель и модель которую нужно обучать, а в `DPO` нужен специальный датасет. Ну это жесть, нужно было что-то попроще и практичнее - `GRPO`. В чем суть GRPO: на каждый пример вашего датасета модель генерирует n ответов (группа) + у вас есть правильный, при помощи reward-функций, считается награда и модель обновляет свои веса максимизируя вероятность предсказывать успешный вариант и штрафуя за плохие.
